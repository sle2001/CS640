...
class Sender:
    # Initialize function
    def __init__(self, port: int, req_port: int, rate: int, seq_num: int, length: int) -> None:
        self.listen_port = port # Port on which the sender waits for requests
        self.req_port = req_port # Port on which the requester is waiting
        ...
        self.UDP_IP = "127.0.0.1" # UDP IP Address
    ...
    def send_file(self, filename: str) -> None:
        ...
        if file_size <= self.length:
            headers.append(struct.pack("!cII", "D".encode(), socket.htonl(self.seq_num), file_size)) # Append the current header
            seq_nums.append(self.seq_num) # Append the current sequence number
            self.seq_num += file_size # Add the file size to the total sequence number
            ...
            for i in range(num_packets):
                ...
                if i == num_packets - 1:
                    # last packet
                    headers.append(struct.pack("!cII", "D".encode(), socket.htonl(self.seq_num), last_payload_length)) # Append the current header
                    seq_nums.append(self.seq_num) # Append the current sequence number
                    self.seq_num += last_payload_length # Add the length to the total sequence number
                else:
                    headers.append(struct.pack("!cII", "D".encode(), socket.htonl(self.seq_num), self.length)) # Append the current header
                    seq_nums.append(self.seq_num) # Append the current sequence number
                    self.seq_num += self.length # Add the length to the total sequence number
        ...
        for i in range(len(header_and_payload)):
            self.sock.sendto(header_and_payload[i], (self.requester_address, self.req_port)) # Send the DATA packet
        ...
        # send END packet
        self.sock.sendto(struct.pack("!cII", "E".encode(), socket.htonl(self.seq_num), 0), (self.requester_address, self.req_port))
        self.log_info("E", self.seq_num, last_payload_length, b"") # Logo info of END packet
    ...
    def log_info(self, type: Literal["D", "E"], seq: int, length: int, payload: bytes) -> None:
        if type == "D":
            ...
            print(f"requester addr: {self.requester_address}:{self.req_port}")
